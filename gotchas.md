...
was which published numbers were at risk, and containment settles part of it
cleanly: the 5-channel linear family contains the 4-channel one (set the new
weight to zero), so any query whose gold was reachable stays reachable, and the
per-query optimum can only rise. The LP reachability figure is therefore a lower
bound and cannot be overturned by restoring the channel.
...
Containment is a property of the object; the sampler has its own
dimension-dependent bias, and they move in opposite directions here.
...
The oracle combiner above had already been retracted once, on exactly the
grounds that it measured a 64-draw sampler rather than a ceiling. The retraction
was written in prose. The number stayed in the ladder table.

It was then quoted as a bound by two people on the same day — including in a
spec written to test whether that bound was family-relative, which is a whole
...
Strike the value in the table itself — delete the row, or replace the figure
with `RETRACTED` and a pointer. If it is still typeset as a number next to its
peers, it is still in circulation whatever the surrounding paragraph says.
